...
package effects 
{
	...
	public class StarfieldFX extends BaseFX
	{
		...
		private var starPos:Vector.<FlxPoint>;
		private var starPrevPos:Vector.<FlxPoint>;
		private var starColors:Vector.<uint>;
		...
		public static const STARFIELD_TYPE_2D_STREAK:int = 3;
		...
		public function create(x:int, y:int, width:uint, height:uint, quantity:uint = 200, type:int = 1, updateInterval:int = 20):FlxSprite
		{
			...
			stars = new Array();
			starPos = new Vector.<FlxPoint>;
			starPrevPos = new Vector.<FlxPoint>;
			starColors = new Vector.<uint>;
			...
			for (var i:uint = 0; i < quantity; i++)
			{
				...
				starPos.push(new FlxPoint(star.x, star.y));
				starPrevPos.push(new FlxPoint(star.x, star.y));
				starColors.push(depthColours[star.speed - 1]);
			}
		...
		private function update2DStarfield():void
		{
			for each (var star:Object in stars)
			{
				...
				while (star.x > sprite.width)
				{
					star.x = star.x - sprite.width;
				}
				while (star.x < 0)
				{
					star.x = sprite.width + star.x;
				}
				
				while (star.y > sprite.height)
				{
					star.y = star.y - sprite.height;
				}
				while (star.y < 0)
				{
					star.y = sprite.height + star.y;
				}
		...
		private function updateStreakField():void
		{
			updateCount = 0;
			for each (var star:Object in stars)
			{
				starPrevPos[updateCount].x = Number(star.x);
				starPrevPos[updateCount].y = Number(star.y);
				
				star.x += (starXOffset * star.speed);
				star.y += (starYOffset * star.speed);
				
				starPos[updateCount].x = Number(star.x);
				starPos[updateCount].y = Number(star.y);
		...
		public function draw():void
		{
			if (getTimer() > tick)
			{
				if(ready){
					...
					{
						update2DStarfield();
					}else if(starfieldType == STARFIELD_TYPE_3D)
					{
						update3DStarfield();
					}else {
						updateStreakField();
						sprite.drawLineBatch(starPrevPos, starPos, starColors, 1);
					}
		...
		public function set starFieldType(value:int):void {
			starfieldType = value;
		}
	}
}